...
type GeneratorFunction<TArgs extends unknown[], T, TReturn, TNext> = (...args: TArgs) => Generator<T, TReturn, TNext>;

type EnhancedGenerator = Generator & { callSite: CallSite };

export const r = <TArgs extends unknown[], T, TReturn, TNext = any>(
...
const isGenerator = (generator: unknown): generator is Generator => 
  ...
  generator !== null &&
  (generator.toString() === '[object Generator]' || generator.toString() === '[object AsyncGenerator]');

const isEnhancedGenerator = (generator: unknown): generator is EnhancedGenerator =>
...
export const disposeRecursive = (context: FrameContext, plugins: PluginInstance[]) => {
...
const keys = <T extends Record<string, unknown>>(value: T) => Object.keys(value) as (keyof typeof value)[];

export const enterScopeAsync = async (generator: unknown, ctx: Fiber, plugins: PluginInstance[]) => {
...
export const enterScopeSync = (generator: unknown, ctx: Fiber, plugins: PluginInstance[]) => {
  if (!isGenerator(generator))
    throw new Error(`Non generator passed to enterScope`);
